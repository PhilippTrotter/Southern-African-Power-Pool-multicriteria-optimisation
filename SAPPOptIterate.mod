...
main {
  var source = new IloOplModelSource("SAPPOpt.mod");
  var cplex = new IloCplex();
  var def = new IloOplModelDefinition(source);
  var opl = new IloOplModel(def,cplex);
  var data = new IloOplDataSource("SAPPOpt.dat");
  var result = new IloOplOutputFile("resultPR50.csv");
